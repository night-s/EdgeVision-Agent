...
int main(int argc, char **argv) {
    ...
    try {
        ...
        while (pipeline.running()) {
            ...
            if (poll(&p, 1, 200) > 0)
                break;
            if (config.run_seconds &&
                FrameClock::now() - start >= std::chrono::seconds(config.run_seconds))
                break;
        }
        pipeline.stop();
        std::cout << "[shutdown] all workers stopped\n";
