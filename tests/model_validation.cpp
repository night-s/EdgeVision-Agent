...
static json encode(const InferenceResult &result) {
    ...
    for (const auto &d : result.detections)
        rows.push_back({{"class_id", d.class_id},
    ...
    try {
        if (argc != 4)
            throw std::runtime_error(
                "Usage: edge_model_validation model.rknn frame-640x480.yuyv output.json");
        ...
        try {
            ...
            report["probability_mode_error"] = e.what();
            report["activation"] = "logits";
        }
        ...
        for (int i = 0; i < 12; ++i) {
            ...
            if (fp.detections.size() != quant.detections.size())
                throw std::runtime_error("float/native detection count mismatch");
            for (size_t k = 0; k < fp.detections.size(); ++k) {
                const auto &a = fp.detections[k];
                const auto &b = quant.detections[k];
                if (a.class_id != b.class_id || a.box != b.box ||
                    std::abs(a.confidence - b.confidence) > 1e-5)
                    throw std::runtime_error("float/native detections differ");
            }
            if (i >= 2) {
                float_ms += fp.output_ms + fp.post_ms;
                native_ms += quant.output_ms + quant.post_ms;
            }
        ...
        try {
            ...
            cv::Mat diff;
            cv::absdiff(rgb, accelerated, diff);
            auto mean = cv::mean(diff);
            double maximum = 0;
            cv::minMaxLoc(diff.reshape(1), nullptr, &maximum);
            ...
            report["cpu_preprocess_ms"] = cpu_ms / 50;
            report["rga_preprocess_ms"] = rga_ms / 50;
            report["rga_available"] = true;
        } catch (const std::exception &e) {
            report["rga_available"] = false;
            report["rga_error"] = e.what();
        }
#endif
