...
#include <atomic>
class VideoService {
 Config config_;Metrics& metrics_;FrameClock::time_point origin_;
 ...
 GMainContext* context_=nullptr;GMainLoop* loop_=nullptr;guint source_=0;
 std::thread video_thread_,rtsp_thread_;
 std::mutex rtsp_mutex_,error_mutex_;
 bool waiting_key_=true;uint64_t rtsp_base_=0;
...
public:
 ...
 void start();
 void stop();
 void submit(FramePtr frame){if(healthy_)frames_.pushLatest(std::move(frame));}
 void trigger(FrameClock::time_point t);
 nlohmann::json recordingStatus(){return recorder_.status();}
 void resetRecording(FrameClock::time_point t){
  recorder_.reset(uint64_t(std::chrono::duration_cast<std::chrono::nanoseconds>(t-origin_).count()));
 }
 bool healthy()const{return healthy_;}
 std::string error(){std::lock_guard<std::mutex> l(error_mutex_);return error_;}
 size_t dropped()const{return frames_.dropped();}
};
