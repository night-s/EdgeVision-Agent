...
class Metrics {
  ...
  public:
    ...
    nlohmann::json snapshot() {
        ...
        j["counts"] = counts_;
        j["sample_window"] = "latest 1024 observations per stage";
        if (elapsed >= 1.0 || cached_rates_.is_null()) {
            ...
            j["cpu_percent_one_core"] = elapsed > 0 ? 100 * (cpu - last_cpu_) / elapsed : 0;
            last_cpu_ = cpu;
            last_ = now;
            last_capture_ = counts_["captured"];
            last_process_ = counts_["processed"];
            last_encode_ = counts_["encoded"];
            cached_rates_ = {{"capture_fps", j["capture_fps"]},
                             {"process_fps", j["process_fps"]},
                             {"encode_fps", j["encode_fps"]},
                             {"cpu_percent_one_core", j["cpu_percent_one_core"]},
        ...
        for (auto &[name, s] : samples_) {
            ...
            auto percentile = [&](double p) {
                return sorted[std::min(sorted.size() - 1,
                                       size_t(std::ceil(p * sorted.size())) - 1)];
            };
            j["stages_ms"][name] = {{"count", s.size()},
                                    {"average", std::accumulate(s.begin(), s.end(), 0.) / s.size()},
                                    {"p50", percentile(.5)},
                                    {"p95", percentile(.95)},
                                    {"maximum", sorted.back()}};
        }
        ...
        j["fd_count"] = descriptors; // Includes the descriptor used while enumerating.
        return j;
    }
};
