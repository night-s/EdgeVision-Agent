...
class Preprocessor {
 ...
 bool rga_;
public:
 Preprocessor(int width,int height,bool rga):box_(Letterbox::make(width,height)),rga_(rga){
  ...
  if(rga)throw std::runtime_error("RGA not enabled in this build");
#endif
 }
...
#ifdef EDGE_WITH_RGA
   auto src=wrapbuffer_virtualaddr_t(raw.data,raw.cols,raw.rows,raw.cols,raw.rows,RK_FORMAT_YUYV_422);
   auto full=wrapbuffer_virtualaddr_t(full_.data,full_.cols,full_.rows,full_.cols,full_.rows,RK_FORMAT_RGB_888);
   auto resized=wrapbuffer_virtualaddr_t(resized_.data,resized_.cols,resized_.rows,resized_.cols,resized_.rows,RK_FORMAT_RGB_888);
   auto status=imcvtcolor(src,full,RK_FORMAT_YUYV_422,RK_FORMAT_RGB_888);
   if(status!=IM_STATUS_SUCCESS)throw std::runtime_error(std::string("RGA color conversion: ")+imStrError(status));
 ...
 const Letterbox& geometry() const {return box_;}
};
